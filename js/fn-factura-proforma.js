...
/* ----------------------------------------------------------------------------------- */
function guardarFacturaProforma(){
	...
	if( idcliente != "" && total != "" && total != 0.00 ){
		...
		$.ajax({
			type:"POST",
			url:"bd/data-factura-proforma.php",
			data:{ encabezado: fencabezado, detalle: fdetalle, reg_factura_p : 1 },
			beforeSend: function () {
				$("#bt_reg_proforma").fadeOut(200);
...
/* ----------------------------------------------------------------------------------- */
function editarFacturaProforma(){
	fencabezado = obtenerVectorEncabezado();
	fdetalle = obtenerVectorDetalle();			// fn-documento.js
	...
	$.ajax({
		type:"POST",
		url:"bd/data-factura-proforma.php",
		data:{ encabezado: fencabezado, detalle: fdetalle, edit_factura_p : 1 },
		beforeSend: function () {			
			$("#bt_edit_proforma").fadeOut( 200 );
			$("#btn_confirmacion").fadeOut( 200 );
		},
		success: function( response ){
			console.log(response);
...
$( document ).ready(function() {
	...
	var cant = ""; 	
	/*===============================================================================*/
    $("#bt_reg_proforma").on( "click", function(){
		$("#closeModal").click();
		if( checkFacturaProforma() == 0 )
			guardarFacturaProforma();
		else
			$("#enl_vmsj").click();
    });

    $("#bt_edit_proforma").on( "click", function(){
	
		$("#closeModal").click();
		if( checkFacturaProforma() == 0 )
			editarFacturaProforma();
		else
			$("#enl_vmsj").click();
    });
    /*===============================================================================*/
