<?php
	...
	function obtenerReporteRelacionGastos( $dbh, $fecha_i, $fecha_f, $idu ){
		$lista_g = array();
		$q = "select idGasto, tipo, beneficiario, concepto, date_format(fecha_pago,'%d/%m/%Y') as fpago, 
		date_format(fecha_registro,'%d/%m/%Y %h:%i %p') as fregistro, 
		banco, monto, monto_pagado, forma_pago, noperacion from gasto 
		where ( fecha_pago BETWEEN '$fecha_i' AND '$fecha_f' ) and tipo='gasto' 
		and idUsuario = $idu order by fecha_pago asc";		
		//echo $q;
	...
	/*-------------------------------------------------------------------------------------------------------*/
	function obtenerReportePagoFacturas( $dbh, $fecha_i, $fecha_f, $idu ){
		$lista_g = array();
		$q = "select g.idGasto as idg, g.beneficiario, c.nfactura as nfactura, 
		date_format(g.fecha_pago,'%d/%m/%Y') as fpago, 
		date_format(g.fecha_registro,'%d/%m/%Y %h:%i %p') as fregistro, 
		g.banco, g.monto, g.forma_pago, g.noperacion   
		from gasto g, compra c where ( fecha_pago BETWEEN '$fecha_i' AND '$fecha_f' ) 
		and g.tipo='pago' and c.idCompra = g.idCompra and g.idUsuario = $idu order by g.fecha_pago asc";		
		//echo $q;
		$data = mysql_query( $q, $dbh );
		while( $g = mysql_fetch_array( $data ) ){
			$lista_g[] = $g;	
	...
	function obtenerReporteFacturasPorCobrar( $dbh, $fecha_i, $fecha_f, $pret, $idu ){
		...
		and idUsuario = $idu and (estado='pendiente' or estado='vencida') order by F.fecha_emision asc";
		//echo $q;
		$data = mysql_query( $q, $dbh );
		while( $f = mysql_fetch_array( $data ) ){
			$lista_f[] = $f;	
	...
	if( isset( $_POST["reporte"] ) ){ 
		...
		if( $nombre_reporte == "pago_facturas" )		
			$reporte_data = obtenerReportePagoFacturas( $dbh, $_POST["f_ini"], $_POST["f_fin"], $idu );
