<?php
	...
	function obtenerReporteRelacionGastos( $dbh, $fecha_i, $fecha_f, $idu ){
		...
		and estado='creado' and idUsuario = $idu order by fecha_pago asc";		
		//echo $q;
	...
	function obtenerReportePagoFacturas( $dbh, $fecha_i, $fecha_f, $idu ){
		...
		g.banco, g.monto, g.monto_pagado, g.forma_pago, g.noperacion   
		from gasto g, compra c  where ( fecha_pago BETWEEN '$fecha_i' AND '$fecha_f' ) 
		and g.tipo='pago' and c.idCompra = g.idCompra and g.idUsuario = $idu order by fpago asc";		
		//echo $q;
		$data = mysql_query( $q, $dbh );
		while( $g = mysql_fetch_array( $data ) ){
			$lista_g[] = $g;	
